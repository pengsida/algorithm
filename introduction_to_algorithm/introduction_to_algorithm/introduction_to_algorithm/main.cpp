...
pthread_mutex_t north_wait_manager_lock;
pthread_mutex_t east_wait_manager_lock;
pthread_mutex_t west_wait_manager_lock;
pthread_mutex_t south_wait_manager_lock;

pthread_mutex_t source_a_manager_lock;
pthread_mutex_t source_b_manager_lock;
pthread_mutex_t source_c_manager_lock;
pthread_mutex_t source_d_manager_lock;
...
void * from_north(void *arg)
{
    ...
    pthread_mutex_lock(&source_c_manager_lock);
    pthread_mutex_unlock(&source_c_manager_lock);
    ...
    if(remaining_sources == 0)
    {
        ...
        pthread_mutex_lock(&north_wait_manager_lock);
        pthread_cond_wait(&north_wait_manager, &north_wait_manager_lock);
        pthread_mutex_unlock(&north_wait_manager_lock);
    ...
    pthread_mutex_lock(&source_d_manager_lock);
    pthread_mutex_unlock(&source_d_manager_lock);
...
void * from_west(void *arg)
{
    ...
    if(remaining_sources == 0)
    {
        ...
        pthread_mutex_lock(&west_wait_manager_lock);
        pthread_cond_wait(&west_wait_manager, &west_wait_manager_lock);
        pthread_mutex_unlock(&west_wait_manager_lock);
    ...
    pthread_mutex_lock(&source_a_manager_lock);
    pthread_mutex_unlock(&source_a_manager_lock);
...
void * from_south(void *arg)
{
    ...
    if(remaining_sources == 0)
    {
        ...
        pthread_mutex_lock(&south_wait_manager_lock);
        pthread_cond_wait(&south_wait_manager, &south_wait_manager_lock);
        pthread_mutex_unlock(&south_wait_manager_lock);
    ...
    pthread_mutex_lock(&source_b_manager_lock);
    pthread_mutex_unlock(&source_b_manager_lock);
...
void * from_east(void *arg)
{
    ...
    if(remaining_sources == 0)
    {
        ...
        pthread_mutex_lock(&east_wait_manager_lock);
        pthread_cond_wait(&east_wait_manager, &east_wait_manager_lock);
        pthread_mutex_unlock(&east_wait_manager_lock);
...
//    request_a_queue.len = 0;
    ...
    {
        pthread_mutex_lock(&source_a_manager_lock);
        pthread_cond_wait(&request_source_a, &source_a_manager_lock);
...
//    request_b_queue.len = 0;
    ...
    {
        pthread_mutex_lock(&source_b_manager_lock);
        pthread_cond_wait(&request_source_b, &source_b_manager_lock);
...
//    request_c_queue.len = 0;
    ...
    {
        pthread_mutex_lock(&source_c_manager_lock);
        pthread_cond_wait(&request_source_c, &source_c_manager_lock);
...
//    request_d_queue.len = 0;
    ...
    {
        pthread_mutex_lock(&source_d_manager_lock);
        pthread_cond_wait(&request_source_d, &source_d_manager_lock);
...
//    int* nums = (int*)malloc(len * sizeof(int));
    ...
    pthread_mutex_init(&north_wait_manager_lock, NULL);
    pthread_mutex_init(&east_wait_manager_lock, NULL);
    pthread_mutex_init(&west_wait_manager_lock, NULL);
    pthread_mutex_init(&south_wait_manager_lock, NULL);
    
    pthread_mutex_init(&source_a_manager_lock, NULL);
    pthread_mutex_init(&source_b_manager_lock, NULL);
    pthread_mutex_init(&source_c_manager_lock, NULL);
    pthread_mutex_init(&source_d_manager_lock, NULL);
